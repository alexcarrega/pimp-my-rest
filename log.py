...
cfg_log = config('console', 'file', 'net', 'logger',
...
def logger(name: str) -> Logger:
    ...
    if cfg_log.console.has('format'):
        _hndl_console.setFormatter(Formatter(cfg_log.console.format))
    if cfg_log.console.has('level'):
        _hndl_console.setLevel(cfg_log.console.level)
    _log.addHandler(_hndl_console)

    _hndl_file = TimedRotatingFileHandler(filename=f'{cfg_log.file.path}/{name}.log',
                                          when=cfg_log.file.when,
                                          interval=cfg_log.file.interval)
    if cfg_log.file.has('format'):
        _hndl_file.setFormatter(NoStyleFormatter(cfg_log.file.format))
    if cfg_log.file.has('level'):
        _hndl_file.setLevel(cfg_log.file.level)
    _log.addHandler(_hndl_file)

    _hndl_net = SocketHandler(host=cfg_log.net.get(
        'host', 'localhost'), port=cfg_log.net.get('port', 8765))
    if cfg_log.net.has('format'):
        _hndl_net.setFormatter(NoStyleFormatter(cfg_log.net.format))
    if cfg_log.net.has('level'):
        _hndl_net.setLevel(cfg_log.net.level)
    _log.addHandler(_hndl_net)

    _log.setLevel(cfg_log.logger.get(name, cfg_log.logger.__default__))
    ...
    def __response(r, ok, error, force={'ok': False, 'error': False, 'exit': True}):
        ...
        if r.ok:
            ...
            if _stderr and not force.get('error', False):
                ...
                _log.error(error)
            _log.warning(f':{cfg_main.icon.exit}: [warn]Exit[/warn] ' +
                         f':{cfg_main.icon.code}: code: [hl]{r.exited}[/hl]')
